            ...
            if (buttons.length == 1 && this.props.iconRight) {
                ...
                if (Platform.OS === 'ios') {
                    ...
                    if (Platform.OS === 'ios') {
                        ...
                        newChildren.push(<View key='btn1' style={{flex: 1, alignItems: 'center', justifyContent: 'flex-start', flexDirection: 'row', marginLeft: -10}}>
                        {React.cloneElement(buttons[0], {style: this.getInitialStyle().toolbarButton, header : true, textStyle: {color: this.getTheme().toolbarTextColor}})}
                        </View>)
                        newChildren.push(<View key='title' style={{flex: 4, alignSelf: 'center', alignItems: 'center', justifyContent: 'center'}}>
                        {[title[0]]}
                        </View>)
                        for (let i = 1; i < buttons.length; i++) {
